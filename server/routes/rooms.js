...
router.post("/join", async (req, res) => {
  try {
    ...
    if (room.participants.length >= 5) {
      return res
        .status(400)
        .json({ message: "Room is full (max 5 participants)" });
    }
  ...
  try {
    ...
    if (!room.admin.equals(req.user._id)) {
      return res
        .status(403)
        .json({
          message: "Only the room admin can approve or reject requests",
        });
    }
...
router.delete("/:roomId", async (req, res) => {
  try {
    ...
    if (!room.admin.equals(req.user._id)) {
      return res
        .status(403)
        .json({ message: "Only the room admin can delete the room" });
    }
