...
export default function CodeEditor({
  ...
  const handleRenameFile = (id, newName) => {
    ...
    if (!file) return;
    const newLanguage =
      file.type === "file" ? getLanguageFromFileName(newName) : "folder";
    setFiles((prev) =>
      prev.map((f) =>
        f.id === id ? { ...f, name: newName, language: newLanguage } : f,
      ),
    );
  ...
  return (
    <div style={{ display: "flex", height: "100%", width: "100%" }}>
      <div
        className={`file-explorer-wrapper ${isFilesOpen ? "open" : ""}`}
        style={{
          height: "100%",
          zIndex: 50,
          display: window.innerWidth > 768 && !isFilesOpen ? "none" : "",
        }}
      >
        <FileExplorer
          files={files}
      ...
      >
        ...
        >
          ...
          {activeFile && (
            ...
            >
              ...
              >
                {LANGUAGES.find((l) => l.value === activeFile.language)
                  ?.label || "Plain Text"}
              </span>
              <span
                style={{
        ...
        {!activeFile ? (
          ...
          >
            <div
              style={{ fontSize: "48px", marginBottom: "16px", opacity: 0.2 }}
            >
              📄
            </div>
